...
class LiteratureSearchSettings(BaseSettings):
    """文献搜索相关配置"""
    ...
    literature_search_api_url: str = Field(
        default="http://localhost:8002/api/v1/search", validation_alias="LITERATURE_SEARCH_API_URL"
    )
    literature_search_top_k: int = Field(
        default=3, validation_alias="LITERATURE_SEARCH_TOP_K"
    )
    literature_search_max_chunks: int = Field(
        default=3, validation_alias="LITERATURE_SEARCH_MAX_CHUNKS"
    )
    literature_search_timeout: int = Field(
        default=120, validation_alias="LITERATURE_SEARCH_TIMEOUT"
    )
    literature_search_chunk_content_limit: int = Field(
        default=1000, validation_alias="LITERATURE_SEARCH_CHUNK_CONTENT_LIMIT"
    )
    literature_search_relevant_content_limit: int = Field(
        default=2000, validation_alias="LITERATURE_SEARCH_RELEVANT_CONTENT_LIMIT"
    )
    literature_search_abstract_limit: int = Field(
        default=2000, validation_alias="LITERATURE_SEARCH_ABSTRACT_LIMIT"
    )

    @property
    def literature_search_dir(self) -> Path:
        """文献搜索结果保存目录"""
        return _project_root / "log" / "literature_search"


settings = LiteratureSearchSettings()
...
def _save_literature_results(result: Dict[str, Any], query: str) -> None:
    """
    保存文献搜索结果到文件（辅助函数）
    ...
    """
    try:
        settings.literature_search_dir.mkdir(parents=True, exist_ok=True)
        safe_query = re.sub(r'[^\w\s-]', '', query).strip()
        safe_query = re.sub(r'[-\s]+', '_', safe_query)[:50]
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        base_filename = f"search_results_{safe_query}_{timestamp}"
        
        json_filepath = settings.literature_search_dir / f"{base_filename}.json"
        ...
        if formatted_text:
            txt_filepath = settings.literature_search_dir / f"{base_filename}.txt"
...
async def search_literature(
    ...
    Args:
        ...
        top_k: 返回的文献数量（默认使用settings中的值）
        max_chunks_per_article: 每篇文献的最大chunk数量（默认使用settings中的值）
        router: LLM router实例（用于翻译和查询拆分，如果为None则使用默认router）
        chunk_content_limit: chunk内容长度限制（默认使用settings中的值）
        relevant_content_limit: 相关内容长度限制（默认使用settings中的值）
        abstract_limit: 摘要长度限制（默认使用settings中的值，设置为较大值以获取完整摘要）
        enable_multi_query: 是否启用多查询模式，将复杂查询拆分为多个子主题分别搜索（默认True）
    
    Returns:
        文献搜索结果字典，如果失败返回None
    """
    if top_k is None:
        top_k = settings.literature_search_top_k
    if max_chunks_per_article is None:
        max_chunks_per_article = settings.literature_search_max_chunks
    if chunk_content_limit is None:
        chunk_content_limit = settings.literature_search_chunk_content_limit
    if relevant_content_limit is None:
        relevant_content_limit = settings.literature_search_relevant_content_limit
    if abstract_limit is None:
        abstract_limit = settings.literature_search_abstract_limit
    ...
    if len(subtopics) == 1:
        ...
        # 保存单查询结果
        if result:
            _save_literature_results(result, search_query)
    ...
    # 保存合并后的结果
    if merged_result:
        _save_literature_results(merged_result, search_query)
...
async def _search_literature_single(
    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            timeout_seconds = settings.literature_search_timeout
            logger.debug(f"子查询 '{query}' 搜索请求参数: top_k={top_k}, max_chunks={max_chunks_per_article}")
            
            async with session.post(
                settings.literature_search_api_url,
                json=payload,
