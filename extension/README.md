...
AI Social Scientist采用VSCode插件 + FastAPI后端的架构：
...
通过右侧对话窗口与AI助手进行日常对话。AI助手会根据您的问题智能选择工具：

- **answer_directly**: 直接回答您的问题
- **search_literature**: 搜索相关文献并保存到工作区
- **design_top_level**: 生成实验顶层设计

### 2. 文献检索功能

当您询问需要文献支持的问题时，AI会自动调用文献检索工具：

1. AI会搜索相关文献
2. 检索结果会自动保存到工作区的 `papers` 目录
3. 每个文献会保存为独立的Markdown文件
4. 同时会生成一个汇总文件

保存的文件格式：
- `{文献标题}_{时间戳}.md` - 单个文献的详细信息
- `search_summary_{时间戳}.md` - 检索结果汇总
...
双击 `SIM_SETTINGS.json` 文件，会打开可视化编辑器，可以：
- 可视化选择 Agent Class
